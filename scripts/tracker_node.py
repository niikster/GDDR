...
class YOLOv8SegmentationNode:
    def __init__(self):
        ...
        path = roslib.packages.get_pkg_dir("testing_kuka")

        # config params of topic
        self.input_topic = rospy.get_param("~input_topic", "/camera/rgb/image_rect_color")
        # self.input_topic = rospy.get_param("~input_topic", "/video_topic")
        ...
        self.conf_thres = rospy.get_param("~conf_thres", 0.70)
        self.iou_thres = rospy.get_param("~iou_thres", 0.25)
        ...
        self.model = YOLO(f"{path}/models/nano/weights/best.pt")
        self.model.fuse()

        # initialize sub and pubs
...
if __name__ == "__main__":
    ...
    rospy.loginfo('YOLO segmentation node has been started.')
    node = YOLOv8SegmentationNode()
    rospy.spin()
    rospy.loginfo('YOLO segmentation node has been killed.')
