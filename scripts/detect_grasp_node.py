...
class GraspDetectionNode:
    ...
    def process_plane(self, plane_cloud, color):
        ...
        try:
            ...
            return plane_cloud, marker, hull_points, average_normal
        except Exception as e:
            rospy.logwarn(f"Convex hull computation failed: {e}")
            return plane_cloud, marker, None, average_normal
    ...
    def detect_grasp_areas(self, cloud):
        ...
        while True:
            if len(cloud.points) < 3:
                break
            plane_model, inliers = cloud.segment_plane(distance_threshold=0.005, ransac_n=3, num_iterations=3000)
            if len(inliers) < 20:
                break
            ...
            for i in range(max_label + 1):
                ...
                cluster, marker, hull_points, average_normal = self.process_plane(cluster, color)

                planes += cluster
                if hull_points is not None:
                    hull_points.colors = o3d.utility.Vector3dVector(np.tile(color, (len(hull_points.points), 1)))
                    contours += hull_points
                    contours_array.append(hull_points)
                    edges, grasp_candidates = self.find_grasp_hypotheses(hull_points, average_normal, color, grasp_marker_array)
